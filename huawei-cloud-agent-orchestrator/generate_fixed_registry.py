...
def generate_service_registry():
    ...
    # 文件头
    output_lines.append('"""')
    output_lines.append('华为云服务注册中心 - 完整版本')
    output_lines.append('包含华为云全量云服务(49个)及完整API操作列表')
    output_lines.append('"""')
    output_lines.append('')
    ...
    output_lines.append('')
    output_lines.append('')
    output_lines.append('@dataclass')
    output_lines.append('class ServiceInfo:')
    output_lines.append('    """服务信息"""')
    output_lines.append('    name: str')
    output_lines.append('    description: str')
    output_lines.append('    module_name: str')
    output_lines.append('    client_class: str')
    output_lines.append('    common_operations: List[str]')
    output_lines.append('')
    output_lines.append('    @property')
    output_lines.append('    def sdk_package_name(self) -> str:')
    output_lines.append('        """SDK包名"""')
    output_lines.append('        return self.module_name')
    output_lines.append('')
    output_lines.append('    @property')
    output_lines.append('    def operations_count(self) -> int:')
    output_lines.append('        """操作数量"""')
    output_lines.append('        return len(self.common_operations)')
    output_lines.append('')
    output_lines.append('    @property')
    output_lines.append('    def operations(self) -> List[str]:')
    output_lines.append('        """操作列表（兼容旧代码）"""')
    output_lines.append('        return self.common_operations')
    output_lines.append('')
    output_lines.append('')
    output_lines.append('class HuaweiCloudServiceRegistry:')
    output_lines.append('    """华为云服务注册中心"""')
    output_lines.append('')
    output_lines.append('    def __init__(self):')
    output_lines.append('        # 预定义的完整服务列表 - 包含华为云全量云服务')
    output_lines.append('        self.services: Dict[str, ServiceInfo] = {')

    # 为每个服务生成ServiceInfo
    for code, (name, module, client, ops) in FULL_SERVICES.items():
        output_lines.append(f'            "{code}": ServiceInfo(')
        output_lines.append(f'                name="{code}",')
        output_lines.append(f'                description="{name}",')
        output_lines.append(f'                module_name="{module}",')
        output_lines.append(f'                client_class="{client}",')
        output_lines.append('                common_operations=[')
    ...
    output_lines.append('        }')
    output_lines.append('')
    output_lines.append('    def get_service(self, service_name: str) -> Optional[ServiceInfo]:')
    output_lines.append('        """获取服务信息"""')
    output_lines.append('        return self.services.get(service_name)')
    output_lines.append('')
    output_lines.append('    def list_services(self) -> List[str]:')
    output_lines.append('        """列出所有服务名称"""')
    output_lines.append('        return list(self.services.keys())')
    output_lines.append('')
    output_lines.append('    def get_all_services(self) -> Dict[str, ServiceInfo]:')
    output_lines.append('        """获取所有服务"""')
    output_lines.append('        return self.services')
    output_lines.append('')
    output_lines.append('')
    output_lines.append('# 全局注册实例')
    output_lines.append('registry = HuaweiCloudServiceRegistry()')
    output_lines.append('')
    output_lines.append('')
    output_lines.append('def get_registry():')
    output_lines.append('    """获取全局服务注册表实例"""')
    output_lines.append('    return registry')
...
if __name__ == "__main__":
    ...
    total_ops = sum(len(ops) for _, _, _, ops in FULL_SERVICES.values())
